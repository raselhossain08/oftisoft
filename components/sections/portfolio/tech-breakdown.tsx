...
function TechCard({ domain, index }: { domain: typeof domains[0], index: number }) {
    return (
        <motion.div
            ...
            className="group relative h-full bg-card/40 backdrop-blur-md border border-border/50 rounded-3xl p-8 hover:bg-card/60 transition-all duration-500 overflow-hidden"
        >
            {/* Top Gradient Line */}
            ...
            <div className="relative z-10">
                ...
                <div className="flex flex-wrap gap-2">
                    {domain.stack.map(tech => (
                        <span key={tech} className="px-2.5 py-1 bg-background/50 border border-border/50 rounded-md text-xs font-medium text-foreground/80">
                            {tech}
                        </span>
                    ))}
                </div>
